            ...
            if src != dst:
                ...
                if G.has_edge(src, dst):
                    D_G.add_node('l_{}_{}'.format(src, dst),
                                 ...
                                 qsize=G.nodes[src]['queueSizes'],
                                 capraw=G.edges[src, dst]['bandwidth']
                                 )
                ...
                for f_id in range(len(T[src, dst]['Flows'])):
                    if T[src, dst]['Flows'][f_id]['AvgBw'] != 0 and T[src, dst]['Flows'][f_id]['PktsGen'] != 0:
                        D_G.add_node('p_{}_{}_{}'.format(src, dst, f_id),
                                     ...
                                     traffraw=T[src, dst]['Flows'][f_id]['AvgBw'],
                                     packraw=T[src, dst]['Flows'][f_id]['PktsGen']
                                     )
...
def hypergraph_to_input_data(hypergraph):
    ...
    return {"traffic": list(nx.get_node_attributes(D_G, 'traffic').values()),
            ...
            "qsize": list(nx.get_node_attributes(D_G, 'qsize').values()),
            "eqlambda": list(nx.get_node_attributes(D_G, 'eqlambda').values()),
            "capraw": list(nx.get_node_attributes(D_G, 'capraw').values()),
            "traffraw": list(nx.get_node_attributes(D_G, 'traffraw').values()),
            "packraw": list(nx.get_node_attributes(D_G, 'packraw').values()),
...
def input_fn_occu(data_dir, shuffle=False, samples=None):
    ds = tf.data.Dataset.from_generator(generator,
                                        ...
                                        output_shapes=(
                                            {"traffic": tf.TensorShape([None]),
                                             ...
                                             "qsize": tf.TensorShape([None]),
                                             "eqlambda": tf.TensorShape([None]),
                                             "capraw": tf.TensorShape([None]),
                                             "traffraw": tf.TensorShape([None]),
                                             "packraw": tf.TensorShape([None]),
